...
class AttendanceHistorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = AttendanceEvent
        fields = ['id', 'timestamp', 'type', 'latitude', 'longitude',
                  'is_correction', 'correction_reason', 'status']
        read_only_fields = fields



class AttendanceCorrectionSerializer(serializers.Serializer):
    """Serializer dla ręcznych korekt obecności - dostępny dla wszystkich pracowników"""
    timestamp = serializers.DateTimeField(required=True)
    type = serializers.ChoiceField(choices=AttendanceEvent.EVENT_TYPE_CHOICES, required=True)
    reason = serializers.ChoiceField(choices=AttendanceEvent.CORRECTION_REASON_CHOICES, required=True)
    latitude = serializers.DecimalField(max_digits=9, decimal_places=6, required=False, allow_null=True)
    longitude = serializers.DecimalField(max_digits=9, decimal_places=6, required=False, allow_null=True)


class AttendanceCorrectionResponseSerializer(serializers.Serializer):
    """Serializer dla odpowiedzi po utworzeniu korekty"""
